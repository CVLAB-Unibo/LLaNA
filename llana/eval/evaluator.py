...
class LLAMAOpenFreeFormClsEvaluator():
    def __init__(self, inputs, output_dir, output_file, llama_pipeline):
        """
        Args:
            inputs: A dictionary containing the results of the evaluation. It contains two keys: "results" and "prompt".
                "prompt": str
        ...
        self.results = inputs
        #self.inference_prompt = inputs['prompt'] # * used to prompt PointLLM
        self.inference_prompt = '<question>'
        self.correct_predictions = 0  
        self.total_predictions = 0 
        self.invalid_responses = 0
        self.response_data = [] # to save all the response data by openaigpt
        ...
        self.llama_prompt =  gpt4_open_free_from_cls_prompt
        self.output_dir = output_dir
        self.output_file = output_file
        self.temp_output_file = self.output_file.replace(".json", "_processed_temp.json")
    ...
    def parse_llama_response_evaluate(self, llama_response):
        llama_response = llama_response.strip()

        cls_result = llama_response[0].upper()
        reason = llama_response[2:] if len(llama_response) > 2 else ""

        if cls_result not in ['T', 'F']:
            self.invalid_responses += 1
            return 0, "INVALID", llama_response
    ...
    def evaluate_result(self, result):
        ...
        ground_truth = result['ground_truth']
        model_output = result['model_output']
        messages = [{"role": "user", "content": self.llama_prompt.format(ground_truth=ground_truth, model_output=model_output)}]
        terminators = [self.llama_pipeline.tokenizer.eos_token_id, self.llama_pipeline.tokenizer.convert_tokens_to_ids("<|eot_id|>")]
        
        prompt = self.llama_pipeline.tokenizer.apply_chat_template(
                messages, 
                tokenize=False, 
                add_generation_prompt=True)
        
        llama_response = self.llama_pipeline(prompt,
            max_new_tokens=256,
        ...
        #completion_tokens = gpt_response['usage']['completion_tokens']
        #gpt_response = gpt_response['choices'][0]["message"]['content']
        # TODO: parse the LLAMA response to get the prompt_tokens and the completion_tokens
        prompt_tokens = 'input prompt'
        completion_tokens = 'completion tokens'

        accuracy, cls_result, reason = self.parse_llama_response_evaluate(llama_response) # return 0, "INVALID", gpt_response if not valid
...
class LLAMACloseSetClsEvaluator(LLAMAOpenFreeFormClsEvaluator):
    def __init__(self, inputs, output_dir, output_file, llama_pipeline):
        super().__init__(inputs, output_dir, output_file, llama_pipeline)
        self.llama_prompt = gpt4_close_set_cls_prompt
        ...
        self.num_categories = len(candidate_lists)
        self.candidate_lists = '\n'.join(candidate_lists)
        self.llama_prompt = self.llama_prompt.format(num_categories=self.num_categories, candidate_lists=self.candidate_lists) + "{model_output}\nOutput: "
    ...
    def parse_llama_response_evaluate(self, llama_response, ground_truth):
        """
        Argument:
            gpt_response: str, index#label#short_reason
            groud_truth: int
        ...
        # * use regular expression to extract
        pattern = r'(\d+#[^#]*#.*$)'
        match = re.search(pattern, llama_response)

        llama_response = match.group(1) if match else llama_response

        llama_response = llama_response.strip()
        llama_response_list = llama_response.split('#')

        cls_result = llama_response_list[0]
        cls_label = llama_response_list[1] if len(llama_response_list) > 1 else ""
        reason = llama_response_list[2] if len(llama_response_list) > 2 else ""
        ...
        try:
            ...
            print(f"Error: unale to parse {llama_response}.")
            cls_result = -1
    ...
    def evaluate_result(self, result):
        ...
        messages = [{"role": "user", "content": self.llama_prompt.format(model_output=model_output)}]
        terminators = [self.llama_pipeline.tokenizer.eos_token_id, self.llama_pipeline.tokenizer.convert_tokens_to_ids("<|eot_id|>")]
        ...
        accuracy, cls_result, cls_label, reason = self.parse_llama_response_evaluate(llama_response, ground_truth) # return 0, "INVALID", gpt_response if not valid
...
class LLAMAObjectCaptioningEvaluator(LLAMAOpenFreeFormClsEvaluator):
    def __init__(self, inputs, output_dir, output_file, llama_pipeline):
        super().__init__(inputs, output_dir, output_file, llama_pipeline)
        self.total_scores = 0
    ...
    def parse_llama_response_evaluate(self, llama_response, ground_truth):
        ...
        llama_score = llama_response_list[0]
        reason = llama_response_list[1] if len(llama_response_list) > 1 else ""

        try:
            # * convert to int
            llama_score = int(llama_score)
            if llama_score not in range(101): # * in 0-100
                # * not valid range
                llama_score = -1
        except ValueError:
            print(f"Error: unale to parse {llama_response}.")
    ...
    def evaluate_result(self, result):
        ...
        llama_score, reason = self.parse_llama_response_evaluate(completion_tokens, ground_truth) # return 0, "INVALID", gpt_response if not valid

        return object_id, model_output, ground_truth, llama_score, reason, prompt_tokens, completion_tokens
...
def start_evaluation_llama(results, output_dir, output_file, eval_type="open-free-form-classification", llama_pipeline=None,
                        parallel=True, num_workers=20):
    """
    Args:
        results: dict or file path to the json file containing the dict
    ...
    if eval_type == "open-free-form-classification":
        evaluator = LLAMAOpenFreeFormClsEvaluator(results, output_dir, output_file, llama_pipeline=llama_pipeline)
    elif eval_type == "modelnet-close-set-classification":
        evaluator = LLAMACloseSetClsEvaluator(results, output_dir, output_file, llama_pipeline=llama_pipeline)
    elif eval_type == "object-captioning":
        evaluator = LLAMAObjectCaptioningEvaluator(results, output_dir, output_file, llama_pipeline=llama_pipeline)
    else:
        raise NotImplementedError(f"eval_type {eval_type} not supported.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results_path", type=str, default="evaluation_results/NeRFLLM_train_stage2/detailed_description_Shapenet.json", help="Path to the results file.")
    ...
    output_file = os.path.basename(args.results_path).replace(".json", f"_evaluated_{args.eval_type}.json")
    ...
    start_evaluation_llama(results=args.results_path, output_dir=args.output_dir, output_file=output_file, eval_type=args.eval_type, llama_pipeline=llama_pipeline, 
