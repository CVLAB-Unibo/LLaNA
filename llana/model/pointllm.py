...
class PointLLMLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if self.point_backbone_type == "PointBERT":
            from llana.model import PointTransformer
    ...
    def forward(
        self,
        input_ids: torch.LongTensor = None,             # * B, L containing all the input_ids
        ...
        point_clouds: Optional[torch.FloatTensor] = None,   # * B, N, 3 with B=64, N=8192, 3=xyz
        ...
        # this block replaces the placeholders of the point tokens with the actual point features, computed by the point backbone and processed by the point projector
        if point_backbone is not None and (input_ids.shape[1] != 1 or self.training) and point_clouds is not None:
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds): # * input_ids: B, L; input_embeds: B, L, C
                ...
                if point_backbone_config['mm_use_point_start_end']:
                    ...
                    point_start_tokens = torch.where(cur_input_ids == point_backbone_config["point_start_token"])[0]    # looks for the start token in the input_ids
                    ...
                    new_input_embeds.append(cur_new_input_embeds)
                    cur_point_idx += 1
            inputs_embeds = torch.stack(new_input_embeds, dim=0)    # new_input_embeds: list of B tensors of shape L, C with B=64, L=626, C=4096
            # check the structure of the first el of new_input_embeds => USER: <p_start> <point_features> <p_end> <question> ASSISTANT:
...
class PointLLMLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        use_cache: Optional[bool] = None, # * control whether to return past_key_values. None
