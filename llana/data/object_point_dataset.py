...
class ObjectPointCloudDataset_Eval(Dataset):
    """Dataset utilities for objaverse."""
    def __init__(self,
                 pcs_path=None,
                 anno_root = None,
        ...
        tokenizer: load point clouds only if None
        """
        super(ObjectPointCloudDataset_Eval, self).__init__()
        ...
        if hst_dataset:
            # read JSON
            self.anno_path = os.path.join('shapenet_text_ds/hst_dataset_filtered_pointllm.json')
            # Load the data list from JSON
    ...
    def _load_objaverse_point_cloud(self, object_id):
        filename = f"{object_id}_{self.pointnum}.npy"
        point_cloud = np.load(os.path.join(self.pcs_path, filename))
    ...
    def __getitem__(self, index):
        ...
        # point exist in the data
        if self.point_indicator in self.filtered_data[index]['conversations'][0]['value']:
            data_dict['point_clouds'] = torch.from_numpy(point_cloud.astype(np.float32))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_path", default="data/objaverse_data", type=str, # sym link to folder 8192_npy (/media/dmercanti/...) with all Objaverse pcs: <model_id>.npy
                        help="Path to the data directory.")
    parser.add_argument("--anno_path", default=None, type=str, required=True,   # path to json with conversations for each model_id
